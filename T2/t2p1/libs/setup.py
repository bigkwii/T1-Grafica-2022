...
def setView(pipeline, mvpPipeline, controller):
    if not controller.camOrtho:
        view = tr.lookAt(
            controller.camPos, #FROM
            controller.camPos + controller.camForward, # TO
            controller.camUp # UP
        )

        projection = tr.perspective(45, float(896)/float(504), 0.1, 100)
        ...
        view = tr.lookAt(
            np.array([0,25,0]), #FROM
            np.array([0,0,0]), # TO
            np.array([-1,0,0]) # UP
        )

        projection = tr.ortho(-10,10,-6,6,0,30)
